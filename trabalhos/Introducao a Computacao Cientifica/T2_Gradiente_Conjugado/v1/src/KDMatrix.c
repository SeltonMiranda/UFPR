...
/**
 * @brief Desaloca memória de uma struct KDMatrix.
 *
 * @param matrix Ponteiro para a struct.
 */
static inline void deallocKDMatrix(KDMatrix* matrix)
{
  int k = matrix->k;
...
ErrorType createKDiagonal(int dimension, int k, KDMatrix* matrix)
{
  if (dimension <= 0 || k <= 0 || k > dimension)
    return ERROR_INVALID_MATRIX_SIZE;

  matrix->k = k;
  if (allocKD(matrix, dimension) != OK)
    return ERROR_MEMORY_ALLOCATION_FAILED;
