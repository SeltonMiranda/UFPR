...
static inline void jacobi(KDMatrix* matrix, real_t* r, real_t* z, int size, real_t* _time, real_t omega)
{
  *_time = timestamp();
  int     k      = matrix->k;
  int     half   = k / 2;
  real_t* diag   = matrix->data + (size_t)half * size;
  int     unroll = size - (size % 4);
  int     i;
    ...
    real_t*         _timeIter)
{
  ...
  // r0 = b - A*x0
  multMatrixVector_otimz(A, x, r, size);
  for (int i = 0; i < size; i++)
  {
    r[i] = b[i] - r[i];
  }
...
// #endif
  ...
  *it           = 0;
  for (int k = 0; k < maxit; k++)
  {
    ...
    real_t denom = dotProduct_otimz(p, Ap, size);
