...
void* alocaMem(int num_bytes) {
    ...
    unsigned long ocupado;
    unsigned long tamanho_do_bloco;
    unsigned long tamanho_necessario = num_bytes + 16; // inclui o cabeçalho
    ...
    while (iterador < topoAtual) {
        ...
        if (ocupado == 0 && tamanho_do_bloco >= num_bytes) {
            *(unsigned long *)((char *)iterador) = 1; // marca como ocupado
            return (void *)((char *)iterador + 16);
        }

        iterador = (void *)((char *)iterador + tamanho_do_bloco + 16);
    }

    // Não encontrou bloco livre suficiente, então aloca novo
    void* inicio_do_bloco = sbrk(tamanho_necessario);
    if (inicio_do_bloco == (void *)-1) {
        return NULL;
    }

    topoAtual = sbrk(0); // atualiza o topo da heap

    // Preenche cabeçalho:
    *(unsigned long *)((char*)inicio_do_bloco) = 1;             // ocupado = 1
    *(unsigned long *)((char*)inicio_do_bloco + 8) = num_bytes; // apenas o tamanho útil
...
void imprimeMapa() {
  ...
  while (iterador < topoAtual) {
    ...
    iterador = (void *)((char *)iterador + tamanho + 16);
  }
