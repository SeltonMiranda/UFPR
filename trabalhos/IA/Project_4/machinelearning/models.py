...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        h = nn.Linear(x, self.w0)
        h = nn.AddBias(h, self.b0)
        h = nn.ReLU(h)
        out = nn.Linear(h, self.w1)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w0.update(grads[0], -self.alpha)
                self.w1.update(grads[1], -self.alpha)
                self.b0.update(grads[2], -self.alpha)
                self.b1.update(grads[3], -self.alpha)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w_out = nn.Parameter(200, len(self.languages))
    ...
    def run(self, xs):
        ...
        for i, x in enumerate(xs):
            if i == 0:
                h = nn.Linear(x, self.wx)
                h = nn.AddBias(h, self.b)
                h = nn.ReLU(h)
            else:
                z_x = nn.Linear(x, self.wx)
                z_h = nn.Linear(h, self.wh)
                z = nn.Add(z_x, z_h)
                z = nn.AddBias(z, self.b)
                h = nn.ReLU(z)
        
        out = nn.Linear(h, self.w_out)
        out = nn.AddBias(out, self.b_out)
        return out
    ...
    def train(self, dataset):
        ...
        while True:
            for xs, y in dataset.iterate_once(self.batch_size):
                ...
                grads = nn.gradients(loss, [self.wx, self.wh, self.b, self.w_out, self.b_out])

                self.wx.update(grads[0], -self.learning_rate)
                self.wh.update(grads[1], -self.learning_rate)
                self.b.update(grads[2], -self.learning_rate)
                self.w_out.update(grads[3], -self.learning_rate)
                self.b_out.update(grads[4], -self.learning_rate)
